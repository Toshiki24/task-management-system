...
> **現在の状況**: MVP機能の実装が完了し、設計書との照合・不具合修正まで完了しています。[テスト項目書](design/test-items.md)の全159項目を自動テスト化し、1回目の実施結果（OK 146 / NG 13）を記録しました。NG項目の修正と再テスト（2回目）はこれから行います。AWSへの本番デプロイも未実施です。
